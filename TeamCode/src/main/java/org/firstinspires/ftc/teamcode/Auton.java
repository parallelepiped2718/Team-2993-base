...
public class Auton extends LinearOpMode { //OpMode
  ...
  public void runOpMode()
  {
    ...
    //robot will start on the right side, facing forward, touching the wall and on the very left edge of the starting tile
    //this placement must be precise for this to work

    setClaw(true);
    goForward(0.5, 15); //15 in forward
    turn(0.5, 90); //90 deg left
    setLift(1.0); //raise arm
    goForward(0.5, 24); //24 in forward
    setLift(-1.0); //lower arm
    sleep(1000);
    setLift(0.0);
    setClaw(false); //release claw
    goForward(0.5, -24); //24 in back
    goSideways(0.5, -12); //12 in left
    goForward(0.5, -36); //36 in back
  }
